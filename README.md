...
### 1. 저장소 클론 및 마켓플레이스 번들 생성

```bash
git clone https://github.com/markncompany/omo.git
cd omo
./scripts/build-marketplace.sh
```

### 2. 사용할 프로젝트에서 플러그인 설치

```bash
cd your-project
claude plugin marketplace add /path/to/omo/dist/omo-marketplace
claude plugin install -s local omo@omo
claude plugin enable -s local omo@omo
```

> `-s local` 옵션을 사용하면 프로젝트의 `.claude/settings.local.json`에 상태가 저장되어 프로젝트별로 플러그인을 관리할 수 있습니다.
...
```bash
claude plugin update -s local omo@omo     # 업데이트
claude plugin disable -s local omo@omo     # 비활성화
```

### 플러그인 유효성 검사

구조 변경 후에는 반드시 검증을 실행하세요.

```bash
claude plugin validate /path/to/omo
```
