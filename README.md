...
OMO는 28개의 스킬 명령어를 제공합니다. 각 명령어는 `/omo:명령어` 형식으로 사용하거나, `#단축키` 패턴으로 빠르게 실행할 수 있습니다.
...
| `#sync` | `/omo:docs-sync [범위]` | README와 유저 가이드가 현재 프로젝트 상태를 반영하는지 검사하고 수정합니다 |
...
| `#st` | `/omo:self-test` | 플러그인 구조, 프론트매터, 버전 일관성을 검증합니다 |
...
| 단축키 | 명령어 | 설명 |
|--------|--------|------|
| `#ev` | `/omo:evolve [포커스]` | 6개 에이전트 병렬 분석 → 우선순위 매트릭스 → 검증된 스프린트 플랜 자동 생성 |
...
| `evolve` | 자동 개선 파이프라인 — 병렬 에이전트 수, 자동 플랜, 메모리 포함 |
...
└── improvements/       # evolve 자동 개선 리포트 및 이력
```
