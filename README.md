...
OMO는 30개의 스킬 명령어를 제공합니다. 각 명령어는 `/omo:명령어` 형식으로 사용하거나, `#단축키` 패턴으로 빠르게 실행할 수 있습니다.
...
| `#uwl` | `/omo:ultrawork-loop [목표]` | ultrawork + ralph-loop 통합 — 작업이 끝날 때까지 멈추지 않습니다 |
...
| `#dh` | `/omo:deep-hunt <증상>` | bug-hunt + deepsearch 병렬 투입으로 버그를 종합적으로 조사합니다 |
...
| `#ev` | `/omo:evolve [포커스]` | 자동 자기개선 파이프라인 — 6개 에이전트 분석 후 스프린트 계획 생성 |
...
├── tests/
│   └── backtest.sh       # 종합 테스트 스위트 (710개 테스트)
├── docs/
